...
function queryParams(){
    ...
    var filters = _.map(this.filters, function(val, key){
        return format("f[%s]=%s", key.toLowerCase(), qs.escape(val));
    });
    var resp = [];
    ...
    var res = resp
        .sort() //this is to ensure correct order for signing purposes
        .join("&");
